...
(function(){
  ...
  async function respond(id,status){
    ...
    if(error)alert('Could not update request: '+error.message);else {load(false);}
  }
  async function start(){
    ...
    channel=db.channel('sq-friend-messages-'+username())
      ...
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'sq_friend_requests',filter:'receiver_username=eq.'+username()},()=>load(false))
      .subscribe();
    clearInterval(timer);timer=setInterval(()=>load(false),10000);
  }
